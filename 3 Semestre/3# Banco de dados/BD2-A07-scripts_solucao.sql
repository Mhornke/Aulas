...
SELECT 
    f.tituloOriginal AS "Filme", 
    g.nome AS "Genero", 
    f.duracao AS "Duração"
FROM filme f
INNER JOIN genero g ON f.idGenero = g.id
WHERE 
    f.idGenero IN (SELECT g.id WHERE g.nome IN ("Suspense")) 
    AND (f.duracao BETWEEN 70 AND 130);
...
DELIMITER $$
CREATE FUNCTION aumenta_duracao(nomeGenero VARCHAR(45)) 
...
SELECT aumenta_duracao ("Suspense");
